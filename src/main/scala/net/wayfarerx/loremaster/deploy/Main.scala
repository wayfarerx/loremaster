...
object Main extends App :
  ...
  override def run(args: List[String]): URIO[ZEnv, ExitCode] =
    val deployment = new TwitterDeployment {}
    ZManaged.fromAutoCloseable(Task(PrintWriter(File(args.head), StandardCharsets.UTF_8.toString))).use { writer =>
      Task.apply(writer.write(emitJson(obj(
      ...
      console.putStrLnErr(Messages.failedToWriteAwsCloudFormationTemplate(thrown)) *> UIO(ExitCode.failure) catchAll {
        _ => UIO.die(thrown)
      }
    }