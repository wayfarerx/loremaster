 ...
 */
case class Retries(backoff: Retries.Backoff, termination: Retries.Termination):

  /**
   * Returns the event and retry backoff to use if the specified event should be retried.
   *
   * @param event The event to evaluate.
   * @return The event and retry backoff to use if the specified event should be retried.
   */
...
object Retries extends ((Retries.Backoff, Retries.Termination) => Retries) :
  ...
  given Data[Retries] = Data.define("Retries") { data =>
  ...
  object Backoff:
    ...
    /** Support for retry backoff policies as configuration data. */
    given Data[Backoff] = Data.define("Retries.Backoff") {
